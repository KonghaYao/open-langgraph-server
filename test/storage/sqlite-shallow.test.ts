/**
 * SqliteShallowSaver 单元测试
 */

import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { SqliteShallowSaver } from '../../src/storage/sqlite/shallow-checkpoint';
...
describe('SqliteShallowSaver', () => {
    let saver: SqliteShallowSaver;
    const testDbDir = join(__dirname, '.test-db');
    const testDbPath = join(testDbDir, `test-shallow-${Date.now()}.db`);
    ...
    beforeEach(async () => {
        ...
        saver = await SqliteShallowSaver.fromConnStringAsync(testDbPath);
    });
